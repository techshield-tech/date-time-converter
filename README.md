...
**Live:** https://techshield-tech.github.io/date-time-converter/

Part of [MMOALL Developer Tools](https://mmoall.com/tools).
Also available at [mmoall.com/tools/date-time-converter](https://mmoall.com/tools/date-time-converter).
...
- [`@mmoall/tool-kit`](https://github.com/techshield-tech/tool-kit) for the shared shell/UI/theme/embed/SEO code
...
├── index.css             # Tailwind, plus theme tokens from @mmoall/tool-kit
...
The shared shell/UI/theme/embed/SEO code (header/footer, theme handling, embed
mode, UI primitives, icons) is no longer part of this repo — it's consumed as
the `@mmoall/tool-kit` npm dependency (see [Tech stack](#tech-stack)).
...
```bash
git clone https://github.com/techshield-tech/date-time-converter.git
cd date-time-converter
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/date-time-converter/**
(note the `/date-time-converter/` path, see [Base path](#base-path)).
...
| otherwise (default)     | `/date-time-converter/` | GitHub Pages                |
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/date-time-converter` repository.
...
`bun run build` with the default `/date-time-converter/` base, and publishes `dist/`.
...
<iframe
  ...
  src="https://techshield-tech.github.io/date-time-converter/?embed=1&theme=dark"
  style="width: 100%; border: 0;"
  title="JSON Formatter"
></iframe>

<script>
  const iframe = document.getElementById('date-time-converter');

  window.addEventListener('message', (event) => {
    const data = event.data;
    if (data?.slug !== 'date-time-converter') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'date-time-converter' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'date-time-converter', height }` | Posted whenever the document height changes (embed mode only) |
